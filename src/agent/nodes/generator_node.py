...
class GeneratorNode:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning("Using fallback model: mistral:7b")
    ...
    def _create_answer_prompt(self) -> PromptTemplate:
        ...
        return PromptTemplate(template=template, input_variables=["question", "documents"])
    ...
    def generate_answer(
        ...
        try:
            ...
            def _generate():
                return self.chain.invoke({"question": question, "documents": combined_docs})
            ...
            docs_used = min(len(documents), 3)
            base = 0.25
            doc_count_bonus = 0.05 * math.log(docs_used + 1)
            quality_bonus = 0.5 * 0.8 if docs_used >= 2 else 0.2
        ...
        def generator_function(state: GraphState) -> GraphState:
            ...
            return {
                **state,
                ...
                "metadata": {**state.get("metadata", {}), "generation_result": result},
                "history": [
                    *state.get("history", []),
                    {
                        "role": "assistant",
